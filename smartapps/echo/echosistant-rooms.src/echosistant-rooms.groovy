	...
	//// LIGHTS INDIVIDUAL FEEDBACK
            fbLights.each { s ->
                ...
                if (tts.contains("$lMatch")) {
                    ...
                    if (tts.contains("on")) {
                        if (fbLightsStatus.contains("on")) {
                            outputTxt = "Yes, the $lMatch is on"
                        }
                        if (fbLightsStatus.contains("off")) {
                            outputTxt = "No, the $lMatch is currently off"
                        }
                    }
                    if (tts.contains("off")) {    
                        if (fbLightsStatus.contains("off")) {
                            outputTxt = "Yes, the $lMatch is off"
                        }
                        if (fbLightsStatus.contains("on")) {
                            outputTxt = "No, the $lMatch is off"
                        }
                    }
                }
                return ["outputTxt":outputTxt, "pContCmds":state.pContCmds, "pShort":state.pShort, "pContCmdsR":state.pContCmdsR, "pTryAgain":state.pTryAgain, "pPIN":pPIN]		
    ...
    //// DOORS INDIVIDUAL FEEDBACK
          ...
          if (tts.contains("check") && tts.contains("light")) { fCommand = "on" }
            ...
            if (tts.contains("check")) {
          	if (tts.contains("lock") || tts.contains("door") || tts.contains("window") || tts.contains("vent") || tts.contains("shades") || tts.contains("blind") || tts.contains("curtain")) {
          		fCommand = "open" }}
			log.debug "fDevice == $fDevice && fValue == $fValue && fCommand == $fCommand"
		...
		// TEMPERATURE //
            ...
            // POWER CONSUMPTION
             ...
             //   currWatts = currWatts*.value.sum()
             //   wattsUsed = wattsUsed*.value.sum()
...
// PARSE OUT TIME FOR DELAYS FOR FIRST COMMAND
    ...
    def newOutPut = tts.replaceAll("\\bin|minutes|minute|${timer}\\b", "");
        if ("$timer" <= 1) {
        	outputTxt = "Ok, I will $newOutPut in $timer minute" // in $timer minutes"
            }
...
def compoundCmd(newTts1, newTts2, switchList, params) {
    ...
    if (("${cmd1}".contains("turned on") || "${cmd1}".contains("turned off")) && ("${cmd2}".contains("turned on") || "${cmd2}".contains("turned off"))) {
   	outputTxt = "I have $cmd1 the $m1 and I have $cmd2 the $m2"
    }
    else if ("${cmd1}"?.contains("turned") || ("${cmd2}" == null)) {
    	outputTxt = "I have $cmd1 the $m1, but I can not $newTts2 due to the compound commands feature currently being limited to individual devices."
        }
    	else if ("${cmd2}"?.contains("turned") || ("${cmd1}" == null)) {
    		outputTxt = "I have $cmd2 the $m2, but I can not $newTts1 due to the compound commands feature currently being limited to individual devices."
        	}
        else {
     	outputTxt = "I'm sorry, but the compound commands feature is currently limited to individual devices, and you have included multiple groups."
        }
    return ["outputTxt":outputTxt, "pContCmds":state.pContCmds, "pShort":state.pShort, "pContCmdsR":state.pContCmdsR, "pTryAgain":state.pTryAgain, "pPIN":pPIN]
}    
